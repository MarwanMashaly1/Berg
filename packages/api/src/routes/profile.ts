...
profileRoutes.get('/connections', async (c) => {
  ...
  const result = await cache.wrap(CK.connections(me.id), TTL.CONNECTIONS, async () => {
    ...
    return { confirmed, pending: incomingRows, sent: sentRows };
  });
...
profileRoutes.get('/circles', async (c) => {
  ...
  const result = await cache.wrap(CK.profileCircles(me.id), TTL.PROFILE_CIRCLES, async () => {
    ...
    if (memberships.length === 0) return { joined: [] };
