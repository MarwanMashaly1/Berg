    ...
    magicLink({
      sendMagicLink: async (data) => {
        ...
        try {
          ...
          console.log(`[Magic Link] Short code: ${shortCode ? '(set)' : 'n/a'}`);

          if (process.env.RESEND_API_KEY) {
            const { Resend } = await import('resend');
            const resend = new Resend(process.env.RESEND_API_KEY);
            const html = await render(MagicLinkEmail({ url: emailLink, email, shortCode }));
            ...
            console.log(`[DEV] No RESEND_API_KEY — magic link omitted from logs`);
          }
        } catch (err) {
    ...
    user: {
      create: {
        after: async (user, ctx) => {
          ...
          const phoneSessionId = (ctx?.request?.headers as Headers | undefined)?.get(
            'x-phone-session-id'
          );
